...
do
	...
	#printf "%-8s %-7s %-30s %-10s%-28s\n" "$user" "$expire" "$passset" "$pw_flags" "$next_pw_ch" >>$log 
	printf "%-8s %-28s\n" "$user" "$next_pw_ch" >> $log 
done

#----------------------------------
# Daily clear /home/download/file and dir
#----------------------------------
rm -f /home/download/dir/*
rm -f /home/download/file/*
